...
export default function CodeOptimisation() {
  ...
  const handleOptimise = () => {
    setResponse(
      `// Optimised Code for ${language} (max length: ${maxLength})\n// ...optimised code here...`
    );
  };
      ...
      <div className="flex flex-col md:flex-row gap-6">
        <div className="flex-1 card p-4 flex flex-col">
          ...
          <textarea
            ...
            placeholder="Paste code to optimise here..."
            spellCheck={false}
          />
        </div>
        <div className="flex-1 card p-4 flex flex-col">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-base font-semibold">Optimised Code</h2>
          </div>
          <pre className="font-mono text-sm flex-1 whitespace-pre-wrap bg-slate-50 rounded-lg p-3 border border-slate-200 min-h-[22rem]">
            {response || <span className="text-slate-400 italic">No optimised code yet.</span>}
          </pre>
        </div>
      </div>
    </div>
